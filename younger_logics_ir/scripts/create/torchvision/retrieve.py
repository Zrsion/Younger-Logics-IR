...
# Last Modified time: 2024-12-13 15:23:51
...
from younger_logics_ir.scripts.commons.torchvision_utils import get_torchvision_model_infos, get_torchvision_model_ids


def save_torchvision_model_infos(save_dirpath: pathlib.Path, force_reload: bool | None = None):
...
def save_torchvision_model_ids(save_dirpath: pathlib.Path):
...
def main(mode: Literal['Model_Infos', 'Model_IDs', 'Model_Inputs'], save_dirpath: pathlib.Path, **kwargs):
    assert mode in {'Model_Infos', 'Model_IDs'}

    if mode == 'Model_Infos':
        save_torchvision_model_infos(save_dirpath, force_reload=kwargs['force_reload'])
        return

    if mode == 'Model_IDs':
        save_torchvision_model_ids(save_dirpath)
        return
