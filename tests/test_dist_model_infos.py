...
# Last Modified time: 2025-01-12 22:55:10
...
def main(model_infos_filename: str, load_dirpath: pathlib.Path, save_dirpath: pathlib.Path, framework: str, limit_l: int, limit_r: int, number_of_file: int):
    gb = 1024*1024*1024
    model_size_limit = (limit_l*gb, limit_r*gb)
    model_infos = list()

    index = 0
    load_model_infos_filepath = load_dirpath.joinpath(f'{model_infos_filename}_{index}.json')
    while load_model_infos_filepath.is_file():
        logger.info(f' -> {index}. Model Infos Load From Dir: \'{load_dirpath}\'.')
        index += 1
        load_model_infos, _ = get_model_infos_and_convert_method(load_model_infos_filepath, framework, model_size_limit)
        model_infos.extend(load_model_infos)
    ...
    for index in range(number_of_file):
        ...
        save_json(save_model_infos, save_dirpath.joinpath(f'{model_infos_filename}_{index}.json'))
        l_index = r_index
